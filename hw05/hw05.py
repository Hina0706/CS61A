...
def move_stack(n, start, end):
    ...
    # recursively
    # base case: n = 1, from start to end
    # recursive: 
        # move n-1 from start to the middle
        # move 1 from start to the end
        # move n-1 from middle to the end
...
def make_withdraw(balance, password):
    ...
    def helper(amount, pw):
        nonlocal balance, error_pw_count
        if len(error_pw_count) >= 3:
            return "Your account is locked. Attempts:" + " " + str(error_pw_count[:3])
        if pw != password:
            error_pw_count.append(pw)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) * upper_bound(y) > 0, 'such interval is meanless!'
